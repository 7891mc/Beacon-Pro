...
SQL;
            try {
                ...
                $dbExists = false;
                try {
                    $check = $pdo->query("SELECT COUNT(*) FROM site_settings");
                    if ($check) {
                        $dbExists = true;
                    }
                } catch (Throwable $e) {
                ...
                if ($dbExists) {
                    // 数据库已存在，删除所有旧表后再重建
                    $tables = ['server_status_logs','gallery_images','gallery_categories','posts','post_categories',
                               'comments','whitelist_applications','friend_links','server_configs','feature_toggles','site_settings','users'];
                    foreach ($tables as $t) {
                        try { $pdo->exec("DROP TABLE IF EXISTS `$t`"); } catch (Throwable $_) {}
                    }
                }
                ...
                $pdo->exec("INSERT INTO site_settings (`key`, value, created_at, updated_at) VALUES
                    ...
                    ('cron_auto_trigger', '1', '$now', '$now'),
                    ('cron_interval_minutes', '1', '$now', '$now')");
        ...
        if (empty($_SESSION['install_step_ok']) || (int) $_SESSION['install_step_ok'] < 3) {
            ...
            try {
                $pdo = installPdo($c);
                $st = $pdo->query("SELECT username, nickname FROM users WHERE role = 'super_admin' AND status = 1 LIMIT 1");
                $existingAdmin = $st ? $st->fetch() : null;
            } catch (Throwable $e) {
                // 表不存在或查询失败，忽略
            }
            
            // 如果是 POST 提交「创建新管理员账号」按钮（删除原有管理员）
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_and_create'])) {
                try {
                    $pdo = installPdo($c);
                    $pdo->exec("DELETE FROM users WHERE role = 'super_admin'");
                    $existingAdmin = null; // 清除已有管理员标记
                    $showCreateForm = true; // 显示创建表单
                } catch (Throwable $e) {
                    $message = '删除原有管理员失败：' . $e->getMessage();
                    $messageType = 'error';
                }
            }
            // 如果是 POST 提交创建新管理员
            ...
            // 如果是 POST 提交跳过（已有管理员）
            elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['skip_admin'])) {
                $_SESSION['install_step_ok'] = 4;
                header('Location: ' . strtok($_SERVER['REQUEST_URI'] ?? 'install.php', '?') . '?s=4');
                exit;
            }
...
<div class="wrap">
    ...
    <?php else: ?>
        ...
        <?php elseif ($step === 2): ?>
            <div class="card">
                ...
                <p style="font-size:13px;color:var(--secondary);margin-bottom:16px;">
                    将创建 12 张数据表并写入默认站点配置、功能开关与服务器配置。<br>
                    <strong style="color:var(--text);">如果数据库中已有同名数据表，将先删除旧表再重建（包含管理员账号），请注意备份。</strong>
                </p>
                <form method="post" action="">
                    <input type="hidden" name="step" value="2">
                    <button type="submit" class="btn">检查并继续</button>
                </form>
            </div>
        ...
        ?>
            <div class="card">
                <h2>管理员账号</h2>
                <?php if ($existingAdmin && !$showCreateForm): ?>
                    <p style="font-size:13px;color:var(--secondary);margin-bottom:16px;">
                        检测到数据库中已存在管理员账号：
                    </p>
                    <div style="background:rgba(0,0,0,0.03);border-radius:10px;padding:14px;margin-bottom:16px;">
                        <div style="font-size:13px;color:var(--text);margin-bottom:4px;">
                            <strong>用户名：</strong><?= h($existingAdmin['username']) ?>
                        </div>
                        <div style="font-size:13px;color:var(--text);">
                            <strong>昵称：</strong><?= h($existingAdmin['nickname'] ?: $existingAdmin['username']) ?>
                        </div>
                    </div>
                    <form method="post" action="" style="margin-bottom:10px;">
                        <input type="hidden" name="step" value="3">
                        <input type="hidden" name="skip_admin" value="1">
                        <button type="submit" class="btn">使用现有账号并继续</button>
                    </form>
                    <form method="post" action="">
                        <input type="hidden" name="step" value="3">
                        <input type="hidden" name="delete_and_create" value="1">
                        <button type="submit" style="width:100%;padding:10px 16px;background:#fff;color:#dc2626;border:1px solid rgba(220,38,38,0.3);border-radius:10px;font-weight:600;font-size:13px;cursor:pointer;font-family:inherit;transition:all 0.15s;">创建新管理员账号（将删除原有账号）</button>
                    </form>
                <?php else: ?>
